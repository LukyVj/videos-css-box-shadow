...
const SHADOW = (
  ...
  blurred: number,
  divider: number,
  isP3Colors: boolean = false,
  isTextShadow: boolean = false
): string[] => {
  ...
  return data.map((c, i) => {
    ...
    if (i === 0) {
      if (isTextShadow) {
        return `${isZero(x + displaced)} ${isZero(y + displaced)} ${isZero(
          Math.round(offsetMultiplier)
        )} ${isP3Colors ? hexToP3(c) : c}`;
      } else {
        return `inset ${isZero(x + displaced)} ${isZero(
          y + displaced
        )} 0 ${isZero(Math.round(offsetMultiplier))} ${
          isP3Colors ? hexToP3(c) : c
        }`;
      }
    }
    ...
    return `${isZero(x + displaced)} ${isZero(y + displaced)} ${
      blurred ? blurred + "Q" : ""
    }${blurred ? blurred + " " : ""}${isP3Colors ? hexToP3(c) : c}`;
  });
};
...
export default function Home() {
  ...
  const [isP3Colors, setIsP3Colors] = useState(false);
  ...
  }, [capture, webcamOn, size, pixelSize, divider, lessColors, isP3Colors]);
      ...
      <div className="h-screen overflow-scroll">
        ...
        <div className="flex flex-col">
          ...
          <div className="grid grid-cols-2">
            ...
            <div>
              <label>P3-Colors</label>
              <input
                type="checkbox"
                checked={isP3Colors}
                onChange={(e) => setIsP3Colors(e.target.checked)}
              />
            </div>
            <div>
